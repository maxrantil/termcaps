...
/*   Updated: 2022/11/09 10:05:54 by mrantil          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_delete(t_term *term)
{
	...
	if (term->index
		&& (term->inp[term->index] == D_QUO || term->inp[term->index] == S_QUO))
		ft_quote_decrement(term, 0);
	row = ft_row_lowest_line(term);
	if (term->nl_addr[row + 1]
		&& (&term->inp[term->index + 1] == term->nl_addr[term->c_row + 1]
